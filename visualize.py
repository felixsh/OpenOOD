...
def get_acc_nc_ood_mean_corr(benchmark_name,
                   acc_split='val',
                   nc_metric='nc1_cdnv',
                   ood_metric='AUROC'):
    ...
    for run_dir in benchmark_dir.glob('run*'):
        ...
        for ckpt_dir in ckpt_dirs:
            with pd.HDFStore(ckpt_dir / 'metrics.h5') as store:
                ...
                for k in ood_keys:
                    ood_df = store.get(k)
                    for key, value in ood_df.items():
                        near_ood[key].append(ood_df.at['nearood', key])
                        far_ood[key].append(ood_df.at['farood', key])
...
def get_acc_nc_ood_corr(benchmark_name,
                        acc_split='val',
                        ood_metric='AUROC'):
    ...
    for run_dir in benchmark_dir.glob('run*'):
        ...
        for ckpt_dir in ckpt_dirs:
            with pd.HDFStore(ckpt_dir / 'metrics.h5') as store:
                ...
                far_ood = []
                acc_list.append(acc)
                for nc_metric in nc_df.keys():
                    nc_dict[nc_metric].append(nc_df[nc_metric])
                for k in ood_keys:
                    ood_df = store.get(k)
                    key = k[1:]
    ...
    return acc_list, nc_dict, nearood_dict, farood_dict, run_id_dict
...
def plot_correlation(benchmark_name,
    ...
    acc, nc, nearood, farood, run_ids = get_acc_nc_ood_corr(benchmark_name,
                                        acc_split=acc_split,
                                        ood_metric=ood_metric)

    labels = ['acc', 'nc', 'nearood', 'farood']
    def plot_corr_method(acc, nc, nearood, farood, run_ids):
        acc_data = np.array(acc)
        # nc_data = np.array(nc[nc_metric])
        nearood_data = np.array([v for v in nearood.values()]).mean(axis=0)
        farood_data = np.array([v for v in farood.values()]).mean(axis=0)
        nc["accuracy"] = acc_data
        nc["nearood"] = nearood_data
        nc["farood"] = farood_data
    ...
    plot_corr_method(acc, nc, nearood, farood, run_ids)
